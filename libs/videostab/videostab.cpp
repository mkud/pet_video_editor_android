...
{
public:
    MotionEstimatorL1Builder(CommandLineParser &command, const string &_prefix = "")
        : IMotionEstimatorBuilder(command), prefix(_prefix) {}
...
int main(int argc, const char **argv)
{
    try
    {
        ...
        if (arg("lin-prog-motion-est") == "yes")
            motionEstBuilder.reset(new MotionEstimatorL1Builder(cmd));
        else
            motionEstBuilder.reset(new MotionEstimatorRansacL2Builder(cmd));

        Ptr<IMotionEstimatorBuilder> wsMotionEstBuilder;
        if (arg("ws-lp") == "yes")
            wsMotionEstBuilder.reset(new MotionEstimatorL1Builder(cmd, "ws-"));
        else
