    ...
    if (packet.stream_index == video_stream_index) {
        ...
        if (ret < 0) {
            ...
            throw std::runtime_error(
                    MyFormatter() << "Error while sending a packet to the decoder - " << ret);
...
bool Decoder::GetFrame(CMatFrame *matFrame) {
    ...
    if (ret == AVERROR(EAGAIN)) {
        ...
        av_packet_unref(&packet);
        throw std::runtime_error(
                MyFormatter() << "Error while receiving a frame from the decoder - " << ret);
        //here is and end error
    }
    sws_scale(swsctx, (const uint8_t *const *) frame->data, frame->linesize, 0, dec_ctx->height,
              matFrame->getFrame(),
...
void Decoder::Init(int iThreadCount, bool bStaySize) {
    ...
    if (ret < 0) {
        throw std::runtime_error(
                MyFormatter() << "Cannot find a video stream in the input file - " << ret);
    }
    ...
    if (bStaySize) {
        ...
        if (m_iHeightRotated * 4 <= m_iWidthRotated * 5) {
            ...
            rectRoi = cv::Rect((m_iWidth - ((m_Rotate % 180) ? 1000 : 800)) / 2,
                               (m_iHeight - ((m_Rotate % 180) ? 800 : 1000)) / 2,
                               (m_Rotate % 180) ? 1000 : 800,
                               (m_Rotate % 180) ? 800 : 1000);
            rectFinishROI = cv::Rect(100, 100, (m_Rotate % 180) ? 800 : 600,
                                     (m_Rotate % 180) ? 600 : 800);
        }
    }

    const AVPixelFormat dst_pix_fmt = AV_PIX_FMT_BGR24;
    swsctx = sws_getContext(dec_ctx->width, dec_ctx->height, dec_ctx->pix_fmt, m_iWidth, m_iHeight,
...
void Decoder::Run(Decoder *obj, CGlobalBuffer *globalBuffer) {
    for (int i = 0;; i++) {
        ...
        try {
            ...
            if (!obj->nextFrame(pCurMat) || (i >= 150)) {
                ...
                globalBuffer->S3_FINISH(i); //will finish S3 from here
                return; //If no one frame - send stream end and quit
            }
            pCurMat->MakeGray();
    ...
    //globalBuffer->S1PUT_TOS2_DecoderPushFrameToNext(NULL);    //After all frames send stream finish.
}
