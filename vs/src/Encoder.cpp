...
void Encoder::Init(int width, int height, AVRational TimeBase, int iRotation, int iCountThread) {
	...
	if (err < 0) {
		throw std::runtime_error(MyFormatter() << "avcodec_open2 Cannot open video encoder for stream - " << err);
	}
...
///////////////////////////////
	...
	ctx = sws_getContext(avctx->width, avctx->height, AV_PIX_FMT_BGR24, avctx->width, avctx->height, AV_PIX_FMT_YUV420P,
	SWS_BILINEAR, 0, 0, 0);
	if (!ctx) {
...
void Encoder::write(Mat &tmp_mat, int iPtsMat) {
	...
	int err = sws_scale(ctx, (const uint8_t * const *) &tmp_mat.data, stride, 0, tmp_mat.rows, sndFrame->data,
			sndFrame->linesize);
...
void Encoder::write2(uint8_t *tmp_mat, int pts) {
	...
	memcpy(sndFrame->data[1], tmp_mat + _m_width_mul_height + _m_width_mul_height_div_4, _m_width_mul_height_div_4);
	memcpy(sndFrame->data[2], tmp_mat + _m_width_mul_height, _m_width_mul_height_div_4);
...
void Encoder::Run(Encoder *obj, CGlobalBuffer* globalBuffer) {
	CMatFrame *pCurMat = 0;
	int wrong_cntr = 0;
	CMatFrame *pPrevMat = globalBuffer->S3GET_EncoderGetFrameForEncoding();    //Let's take the first frame.
	if (g_bCannotStabilize || !pPrevMat) {
		//left if nothing came
		obj->release();
		return;
	}
	...
	obj->write(pPrevMat->getMatROI(), pPrevMat->getPts());       //Let's draw it.
	for (int i = 3;; i++) {
		try {
			...
			if (!pCurMat) {	//if you get the end of the stream, then we finish
				globalBuffer->S3PUT_TO_S2_EncoderReturnUnusedFrame(pPrevMat);
				obj->release();
				return;
			}
			g_CurrentFrame = pCurMat->getIFrameNum(); // celebrate progress

			//if we have a big difference between the two frames
			if (abs(pPrevMat->GetDiff() - pCurMat->GetDiff()) > 45) {
				//and if the discrepancy is already framed for 3 frames in a row, then it will not converge ...
				if (wrong_cntr > 2) {
				...
				wrong_cntr++; //count this split frame
				pPrevMat->setPts(pCurMat->getPts());    //Mask the old frame to the new
				pPrevMat->setIFrameNum(pCurMat->getIFrameNum()); //completely disguise

				//write it down. (Yes, yes, what a horror, we will record the old frame two times, throwing out the unrecognized one)
				...
				//and for now let’s leave - because it has become new.
				//accordingly, a new one must be returned
				globalBuffer->S3PUT_TO_S2_EncoderReturnUnusedFrame(pCurMat);
			} else {
				wrong_cntr = 0; //reset the bad frame counter
				if ((g_CountFrames - i) < 4) {
					Mat xxx = (pCurMat->getMatROI() * (float(g_CountFrames - i) / 4))
							+ (MatBaseFrame * (float(1) - (float(g_CountFrames - i) / 4)));
					obj->write(xxx, pCurMat->getPts());
				} else
					obj->write(pCurMat->getMatROI(), pCurMat->getPts());    //write a new frame

				//give the old frame to the buffer
				globalBuffer->S3PUT_TO_S2_EncoderReturnUnusedFrame(pPrevMat);

				pPrevMat = pCurMat; //Save the new frame as the old one.
			}
	...
	// obj->release();    //If you went through all the planned frames - finish.
}
